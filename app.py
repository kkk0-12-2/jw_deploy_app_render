...
st.title('📅 AIシフト作成アプリ (土日1人・出勤日数重視版)')
...
st.info(f"設定確認：1日は **{start_wd}曜日** です。土日は1人、火曜は5人、他平日は3人を目標に計算します。")
...
if st.button('この条件でシフトを自動生成する'):
    ...
    A = 3500  # 出勤日数を守る (最優先)
    B = 1200  # 希望休・必須
    C = 50    # 人数制約 (土日・火曜のメリハリをつけるために少し強めに)
    E = 20    # 連勤抑制
    ...
    for d in range(num_days):
        ...
        if current_wd in ['土', '日']:
            ...
            daily_target = 5  # 火曜は5人
        else:
            daily_target = 3  # 他の平日は3人
